...
def get_analysis(s):
    try:
        ...
        prev_roe = "N/A"
        try:
            yf_fin = tk.financials
            yf_bs = tk.balance_sheet
            if not yf_fin.empty and not yf_bs.empty:
                # 尝试抓取上一年
                idx = 1 if len(yf_fin.columns) > 1 else 0
                net_inc = yf_fin.loc['Net Income'].iloc[idx]
                equity = yf_bs.loc['Stockholders Equity'].iloc[idx]
                if equity > 0: prev_roe = round((net_inc / equity) * 100, 1)
        except: pass
        ...
        try:
            ...
            if not e_dates.empty:
                ...
                if not fut.empty:
                    next_obj = fut.index[0]
                    next_e_date, next_e_days = next_obj.strftime('%Y-%m-%d'), (next_obj - now_utc).days
                # 上个财报回顾
                pst = e_dates[e_dates.index < now_utc]
                if not pst.empty:
                    prev_obj = pst.index[0]
                    prev_e_date = prev_obj.strftime('%Y-%m-%d')
                    prev_eps_est = pst['EPS Estimate'].iloc[0]
                    prev_eps_act = pst['Reported EPS'].iloc[0]
        ...
        return {
            ...
            "_p": p, "_m": m200_val, "_h": h, "_m_s": h['Close'].rolling(200).mean(), "_target": target, "_up_val": upside,
...
def render_report(s):
    ...
    with er2:
        surp_v = s['_p_surp']
        surp_color = "green" if (isinstance(surp_v, (int, float)) and surp_v > 0) else "red"
        st.markdown(f"**{t.get('prev_earn_label').format(date=s['_prev_e'])}**")
        st.write(t.get('eps_vs_est').format(act=s['_p_act'], est=s['_p_est'], surp=f":{surp_color}[{surp_v}]"))
    ...
    with st.expander(t.get('risk_title'), expanded=True):
        r1, r2 = st.columns(2)
        r1.success(f"✅ Inst Held {s['_inst']:.1f}% | Short: {s['Short%']}")
...
if search_ticker:
    ...
    else: st.error("Ticker not found. (Data provider delay, please retry)")
